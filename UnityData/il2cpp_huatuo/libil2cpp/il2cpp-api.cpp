...
void managed_init()
{
    auto ass = Assembly::Load("Assembly-CSharp-firstpass.dll");
    auto image = Assembly::GetImage(ass);
    auto huatuoHelperClazz = Class::FromName(image, "Huatuo", "HuatuoHelper");
...
int il2cpp_init(const char* domain_name)
{
    ...
    bool ret = Runtime::Init(domain_name);
...
int32_t il2cpp_huatuo_register_valuetype_shared_inst (const Il2CppType *type) {
    Il2CppClass *klass = Class::FromIl2CppType(type);
    if (klass->instance_size < MAX_SHARED_VALUETYPES)
        huatuo::SHARED_VALUETYPES[klass->instance_size] = &klass->byval_arg;
    else
    {
        std::cerr << "instance size: " << klass->instance_size << ", max support instance size: " <<  MAX_SHARED_VALUETYPES << std::endl;
    }
