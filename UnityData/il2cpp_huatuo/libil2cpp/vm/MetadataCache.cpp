...
static const Il2CppGenericInst* GetSharedInst(const Il2CppGenericInst* inst)
{
    ...
    for (uint32_t i = 0; i < inst->type_argc; ++i)
    {
        ...
        else
        {
            ...
            {
                auto klass = il2cpp::vm::Class::FromIl2CppType(type);
                if (huatuo::metadata::IsInterpreterType(klass))
                {
                    il2cpp::vm::Class::SetupFields(klass);
                    if (klass->instance_size < MAX_SHARED_VALUETYPES &&
                        huatuo::SHARED_VALUETYPES[klass->instance_size] != nullptr) {
                        type = huatuo::SHARED_VALUETYPES[klass->instance_size];
                    } else
                    {
                        std::cerr << klass->namespaze << "@" << klass->name << ", instance size: " << klass->instance_size << std::endl;
                    }
                }
            }
